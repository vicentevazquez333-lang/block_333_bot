"""
Consultas a la base PNB en SQLite (solo lectura).

Genera la base con: python import_pnb_sqlite.py
Ruta por defecto: BASE DE DATOS/pnb.sqlite

Variables de entorno:
  PNB_DB             — ruta al .sqlite
  PNB_DOWNLOAD_URL   — descarga si falta el archivo
  PNB_DOWNLOAD_TOKEN — opcional
  PNB_DOWNLOAD_AUTH  — opcional: bearer
"""
...
_DEFAULT_DB = _PROJECT_ROOT / "BASE DE DATOS" / "pnb.sqlite"


def _raise_bad_github_release_url() -> None:
    raise FileNotFoundError(
        "PNB_DOWNLOAD_URL no debe ser la página del release (/releases/tag/…).\n"
        "Usa el enlace directo al archivo .sqlite (/releases/download/…/pnb.sqlite)."
    )
...
def _assert_sqlite_file(path: Path) -> None:
    ...
    try:
        ...
        pass
    raise FileNotFoundError(
        "El archivo configurado en PNB_DB no es un SQLite válido."
    )


def db_path() -> Path:
    override = os.environ.get("PNB_DB", "").strip()
    return Path(override) if override else _DEFAULT_DB


def _download_pnb_db(url: str, dest: Path) -> None:
    ...
    if part.exists():
        part.unlink()
    try:
        headers = {"User-Agent": "pnb-bot/1"}
        token = os.environ.get("PNB_DOWNLOAD_TOKEN", "").strip()
        if token:
            if os.environ.get("PNB_DOWNLOAD_AUTH", "token").lower() == "bearer":
...
def ensure_pnb_database() -> None:
    path = db_path()
    if path.is_file():
        _assert_sqlite_file(path)
        return
    url = os.environ.get("PNB_DOWNLOAD_URL", "").strip()
    if not url:
        return
    if "/releases/tag/" in url:
        _raise_bad_github_release_url()
    with _download_lock:
        if path.is_file():
            return
        try:
            _download_pnb_db(url, path)
        except urllib.error.HTTPError as e:
            raise FileNotFoundError(
                f"Error HTTP {e.code} al descargar la base PNB."
            ) from e
        except urllib.error.URLError as e:
            raise FileNotFoundError(
                f"No se pudo descargar la base PNB: {e.reason}"
            ) from e
...
def _connect_readonly() -> sqlite3.Connection:
    ...
    if not path.is_file():
        raise FileNotFoundError(
            ...
            "Opciones: python import_pnb_sqlite.py en local, "
            "o PNB_DOWNLOAD_URL + PNB_DB en la nube."
        )
