"""
Importa CICPC.pdf a SQLite para consultas por cédula o nombre.

Uso:
    python import_cicpc_sqlite.py

Opciones:
    --pdf RUTA     Archivo PDF fuente (default: CICPC.pdf)
    --db RUTA      Salida SQLite (default: BASE DE DATOS/cicpc.sqlite)
...
DEFAULT_PDF = ROOT / "CICPC.pdf"
DEFAULT_DB = ROOT / "BASE DE DATOS" / "cicpc.sqlite"
BATCH_SIZE = 5_000
...
def _clean_name(name: str) -> str:
    ...
    # Evita capturar basura del encabezado repetitivo del PDF.
    blocked = (
...
def extract_pdf_text(pdf_path: Path) -> str:
    ...
    text = "\n".join(chunks)
    # Quita marcas de paginación del extractor.
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Importar CICPC.pdf a SQLite indexado.")
    ...
    try:
        ...
        for i in range(0, len(rows), BATCH_SIZE):
            ...
            conn.executemany(
                "INSERT INTO cicpc VALUES (?,?,?,?,?,?,?,?)",
                batch,
            )
            conn.commit()
            if i and i % (BATCH_SIZE * 4) == 0:
                print(f"  … {min(i + len(batch), len(rows)):,} filas")

        print(f"Insertadas: {len(rows):,} filas")
        print("Creando índices…")
        conn.execute("CREATE INDEX idx_cicpc_cedula ON cicpc(cedula)")
        conn.execute("CREATE INDEX idx_cicpc_nombre ON cicpc(nombre COLLATE NOCASE)")
        conn.commit()

        conn.execute("PRAGMA analysis_limit = 400")
        conn.execute("ANALYZE cicpc")
        conn.commit()
